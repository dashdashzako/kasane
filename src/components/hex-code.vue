...
const { hexCode } = defineProps<{ hexCode: string }>()

const { copy, isSupported } = useClipboard({ source: hexCode })
</script>
...
<template>
  <button
    v-if="isSupported"
    @click="copy(hexCode)"
    :aria-label="`Copy ${hexCode} value to clipboard`"
  >
    <span>{{ hexCode }}</span>
...
button {
  ...
  &:hover,
  &:focus-visible {
    background-color: oklch(from var(--color) calc((0.6 - l) * infinity) 0 0 / 0.1);
    outline-color: oklch(from var(--color) calc((0.6 - l) * infinity) 0 0 / 0.5);
